#!/usr/bin/env python3
"""Soft-fail flash_attn → SDPA for ConsistCompose BAGEL qwen2_navit (sm_120 Blackwell).
...
def main() -> None:
    ...
    ap.add_argument(
        "consist_root",
        type=pathlib.Path,
        help="Path to cloned ConsistCompose repo root",
    )
    args = ap.parse_args()
    navit = (
        args.consist_root
    ...
    if not navit.is_file():
        raise SystemExit(f"[patch_flash_sdpa] missing {navit}")
    patch_file(navit)
