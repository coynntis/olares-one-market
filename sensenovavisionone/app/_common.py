"""Shared helpers for SenseNova Vision Gradio + FastAPI servers."""
...
LOG = logging.getLogger("sensenovavision")
...
def append_bootstrap(msg: str) -> None:
    app = os.getenv("APP_NAME", "sensenovavisionone")
    path = Path(os.getenv("BOOT_ATTEMPTS_FILE", f"/workspace/{app}/bootstrap.log"))
...
def set_ready_phase(detail: str = "ready") -> None:
    app = os.getenv("APP_NAME", "sensenovavisionone")
    phase = Path(os.getenv("BOOT_PHASE_FILE", f"/workspace/{app}/.boot-phase"))
    phase.write_text(
