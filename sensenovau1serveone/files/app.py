"""SenseNova-U1 FastAPI + Gradio: t2i, vqa, editing, interleave (single loaded model)."""
...
# Hugging Face file sizes (approx) + usage hints for smthem merger repo.
GGUF_VARIANT_GUIDE: dict[str, dict[str, Any]] = {
    "SenseNova-U1-8B-MoT-8step-Q4_K_S.gguf": {
        "approx_gb": 13.9,
        "role": "8-step distilled + Q4 — fastest, lowest VRAM; try num_steps≈8.",
    },
    "SenseNova-U1-8B-MoT-Q6_K.gguf": {
        "approx_gb": 16.0,
        "role": "Standard merger Q6 — default balance of quality vs footprint on ~24GB.",
    },
    "SenseNova-U1-8B-MoT-SFT-fm-Q6_K.gguf": {
        "approx_gb": 16.1,
        "role": "SFT-fm Q6 — use when you want the instruction-tuned merger variant at Q6.",
    },
    "SenseNova-U1-8B-MoT-SFT-fm-Q8_0.gguf": {
        "approx_gb": 20.0,
        "role": "SFT-fm Q8 — highest quality of these; heaviest VRAM.",
    },
}
...
app = FastAPI(title="SenseNova-U1 serve", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
...
def capabilities():
    ...
    return {
        ...
        "attention_backend_notes": (
            "OSS SenseNova path uses flash_attn.flash_attn_func when ATTENTION_BACKEND=flash or auto+importable flash_attn "
        ...
        "notes": [
            "Optional body field gguf_filename (basename) switches merger GGUF; triggers full reload when different.",
            ...
            "For *-8step-*.gguf checkpoints, try num_steps≈8 (Comfy defaults); standard Q6/Q8 often use more steps.",
        ],
    }
...
def build_gradio() -> gr.Blocks:
    ...
    with gr.Blocks(title="SenseNova-U1") as demo:
        gr.Markdown(
            "# SenseNova-U1\n"
