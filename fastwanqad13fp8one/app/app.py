...
try:
    ...
    if not fastwan_worker_patch.apply(source="app.py import"):
        logging.getLogger("fastwanqad13fp8one").error(
...
MODEL_ID = os.getenv("FASTWAN_MODEL", "FastVideo/FastWan-QAD-FP8-1.3B")
TAEHV_CHECKPOINT = os.getenv("TAEHV_CHECKPOINT", "/models/taehv/taew2_1.pth")
TAEHV_DEVICE = os.getenv("FASTWAN_TAEHV_DEVICE", "cpu")
FP8_GRANULARITY = os.getenv("FASTWAN_FP8_GRANULARITY", "tensor")
...
_LOG = logging.getLogger("fastwanqad13fp8one")


def _verify_fp8_attention_backend() -> None:
    """FP8 QAD uses SageAttention2++ (SAGE_ATTN) — Ampere/Ada/Hopper/Blackwell."""
    try:
        from sageattention import sageattn  # noqa: F401
    except ImportError as exc:
        raise gr.Error(
            "sageattention (SageAttention2++) is not installed. "
            "FastWan-QAD-FP8-1.3B requires it — check /workspace/fastwan-sageattention-build.log"
        ) from exc
    ...
    if "sage_attn.SageAttentionBackend" not in backend_cls:
        raise gr.Error(f"SAGE_ATTN backend unavailable (got {backend_cls}).")
...
def _build_generator() -> VideoGenerator:
    ...
    # Match FastVideo fp8_wan2_1_1_3b.py — FP8 via transformer_quant, not PipelineConfig overrides.
    fp8_quant = get_quantization_config("FP8")(granularity=FP8_GRANULARITY)
...
def _ensure_ready() -> VideoGenerator:
    ...
    if cap[0] < 8:
        raise gr.Error(
            f"FastWan FP8 needs sm89+ (RTX 4090 / Ada / Hopper / Blackwell). Got {cap[0]}.{cap[1]}."
        )

    if not os.path.isfile(TAEHV_CHECKPOINT):
        raise gr.Error(f"Missing TAEHV weights at {TAEHV_CHECKPOINT}")

    _verify_fp8_attention_backend()
    _LOG.info(
        "loading FastWan FP8 model=%s fp8_granularity=%s compile=%s "
        "text_encoder_cpu_offload=%s taehv_device=%s",
        MODEL_ID,
        FP8_GRANULARITY,
        ENABLE_COMPILE,
...
def main() -> None:
    ...
    if PRELOAD:
        _LOG.info("preloading FastWan FP8 on main thread before Gradio")
        try:
            _ensure_ready()
        except Exception:
    ...
    with gr.Blocks(title="FastWan QAD FP8 1.3B One") as demo:
        gr.Markdown(
            "# FastWan QAD FP8 1.3B One\n"
            "**4090 / Ada fallback** — FP8 linear layers + SageAttention2++. "
            "Use when NVFP4 flagship/SA2 OOM on 24GB. ~3.4s for 5s 480p on RTX 4090; "
            "works on Olares One too.\n\n"
            "3-step QAD text-to-video, 81 frames @ 16fps (832×480)."
        )
        prompt = gr.Textbox(
            label="Prompt",
            lines=4,
