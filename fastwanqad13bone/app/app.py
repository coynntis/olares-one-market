...
try:
    ...
    if not fastwan_worker_patch.apply(source="app.py import"):
        logging.getLogger("fastwanqad13bone").error(
...
MODEL_ID = os.getenv("FASTWAN_MODEL", "FastVideo/FastWan-QAD-1.3B")
...
DIT_LAYERWISE_OFFLOAD = os.getenv("FASTWAN_DIT_LAYERWISE_OFFLOAD", "0").lower() in (
    "1",
    "true",
    "yes",
)
PRELOAD = os.getenv("FASTWAN_PRELOAD", "0").lower() in ("1", "true", "yes")
SKIP_BACKEND_VERIFY = os.getenv("FASTWAN_SKIP_BACKEND_VERIFY", "0").lower() in (
    "1",
    "true",
    "yes",
)
TE_GPU_ENCODE = os.getenv("FASTWAN_TE_GPU_ENCODE", "0").lower() in ("1", "true", "yes")

_GENERATOR = None
_TAEHV = None
_LOCK = threading.Lock()
_LOG = logging.getLogger("fastwanqad13bone")


def _verify_attn_qat_backend() -> None:
    """Flagship QAD model needs modified SageAttention3 FP4 (ATTN_QAT_INFER)."""
    from fastvideo.attention.backends.attn_qat_infer import is_attn_qat_infer_available

    if not is_attn_qat_infer_available():
        raise gr.Error(
            "attn_qat_infer kernel not available. Build fastvideo-kernel on Blackwell — "
            "see /workspace/fastwan-kernel-build.log"
        )

    if SKIP_BACKEND_VERIFY:
        _LOG.info("attention backend: ATTN_QAT_INFER (install-time verify skipped)")
        return
    ...
    if "AttnQatInferBackend" not in backend_cls:
        raise gr.Error(f"ATTN_QAT_INFER backend unavailable (got {backend_cls}).")
    _LOG.info("attention backend OK: ATTN_QAT_INFER (SA3 FP4) -> %s", backend_cls)
...
def _release_parent_cuda_cache() -> None:
    """Free parent-process VRAM before spawning the FastVideo worker (shared 24GB GPU)."""
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
    _log_gpu_memory("parent before worker spawn")
...
def _build_generator() -> VideoGenerator:
    ...
    te_precision = os.getenv("FASTWAN_TEXT_ENCODER_PRECISION", "bf16")
    ...
    pipeline_config.text_encoder_precisions = (te_precision,)
...
def _ensure_taehv() -> TaehvDecoder:
    ...
    if _TAEHV is None:
        ...
        _log_gpu_memory("before TAEHV load")
        _TAEHV = TaehvDecoder(TAEHV_CHECKPOINT)
        _log_gpu_memory("after TAEHV load")
    return _TAEHV


def _unload_taehv() -> None:
    """Drop TAEHV from parent GPU so FastVideo worker can use full VRAM for denoise."""
    global _TAEHV
    if _TAEHV is None:
        return
    del _TAEHV.model
    ...
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
        torch.cuda.synchronize()
    _log_gpu_memory("after TAEHV unload")


def _reset_generator() -> None:
    global _GENERATOR, _TAEHV
...
def _run_warmups(gen: VideoGenerator) -> None:
    ...
    with _silence_request_log():
        for i in range(WARMUPS):
            ...
            _release_parent_cuda_cache()
            _log_gpu_memory(f"before warmup {i + 1} denoise")
            try:
                warm = gen.generate(request=_generation_request("warmup", steps=2))
                taehv = _ensure_taehv()
                taehv.decode(fastwan_worker_patch.extract_latents_cpu(warm))
                fastwan_worker_patch.shutdown_worker_after_generate(gen)
                torch.cuda.synchronize()
            except Exception:
                _log_gpu_memory(f"after warmup {i + 1} failure")
                _LOG.exception("warmup %s/%s failed", i + 1, WARMUPS)
                raise
            finally:
                _unload_taehv()
            _log_gpu_memory(f"after warmup {i + 1}")
...
def _ensure_ready() -> VideoGenerator:
    ...
    _verify_attn_qat_backend()
    _LOG.info(
        "loading FastWan generator model=%s compile=%s text_encoder_cpu_offload=%s "
        "te_gpu_encode=%s dit_layerwise_offload=%s te_precision=%s taehv_device=%s",
        MODEL_ID,
        ENABLE_COMPILE,
        TEXT_ENCODER_CPU_OFFLOAD,
        TE_GPU_ENCODE,
        DIT_LAYERWISE_OFFLOAD,
        os.getenv("FASTWAN_TEXT_ENCODER_PRECISION", "bf16"),
        TAEHV_DEVICE,
    )
    _log_gpu_memory("before generator load")
    _GENERATOR = _build_generator()
    _log_gpu_memory("after generator load")
    if WARMUPS > 0:
...
def generate_video(prompt: str, progress=gr.Progress()):
    ...
    with _LOCK:
        ...
        try:
            ...
            err = str(exc).lower()
            if "out of memory" in err or "cuda error" in err:
                _log_gpu_memory("CUDA OOM during generate")
                raise gr.Error(
                    "GPU out of memory during denoise. On 24GB try fastwanqad13fp8one, "
                    "or enable FASTWAN_DIT_LAYERWISE_OFFLOAD=1 in app settings."
                ) from exc
...
def main() -> None:
    ...
    if PRELOAD:
        ...
        _LOG.info("lazy load — model loads on first Generate (saves VRAM at startup)")

    with gr.Blocks(title="FastWan QAD 1.3B One") as demo:
        gr.Markdown(
            "# FastWan QAD 1.3B One\n"
            "3-step QAD text-to-video — ~1.8s for 5s 480p on RTX 5090M "
            "(NVFP4 + SageAttention3 FP4 / ATTN_QAT_INFER)."
        )
        prompt = gr.Textbox(
            label="Prompt",
            lines=4,
